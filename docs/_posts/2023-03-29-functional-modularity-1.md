...
But if 'cons' is the breaking up, what is the glue to put things back together, and how do we use it? The answer lies in a class of functions common to functional programming known as higher-order functions. A higher order function is a function that may take another function as one or more of its arguments and may return a function as the result. In the next section, we introduce the higher order function foldRight and we will se how it glues together the idea of substituting in an accumulator function for 'cons' and then generalizing that accumulation function to any accumulator operation, completing our picture of modularization.
